...
stateDiagram-v2
    ...
    Formulating --> Formulating : генерация идёт,<br/>ничего не произошло
...
**Петля в `Formulating` — зеркало предыдущей дыры, и нашла её реализация.** Генерация
занимает реальное время, а реализация, шагающая автоматом на каждый входящий чанк аудио,
обязана иметь легальное действие для «всё ещё генерируем, ничего не произошло». У всех
остальных состояний такая петля была (`greetingPlays`, `idleTicks`, `userKeepsTalking`,
`speechPlays`, `closingPlays`), у `Formulating` — нет: только `draftReady` и
`draftAbandoned`, то есть каждый тик был обязан завершить состояние.

Проверяющий этого поймать не мог: он волен просто не делать шаг, поэтому у него ничего
не зависало. Реальное время такой свободы не даёт. Это ровно тот класс расхождения, который
ловится только столкновением модели с работающим кодом.
