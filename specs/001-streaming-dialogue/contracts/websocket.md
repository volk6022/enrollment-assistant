...
**Частота дискретизации на проводе — `AUDIO_SAMPLE_RATE` (16 кГц), в обе стороны.** Silero
синтезирует на `TTS_SAMPLE_RATE` (48 кГц), и понижает его **сервер**, в `SileroWorker`, перед
отправкой. Клиент никогда не ресемплит: обе стороны держат один `AudioContext` на 16 кГц и
перекладывают ресемплинг на аппаратный тракт браузера. Отдать на провод 48 кГц, понадеявшись,
что клиент разберётся, — это тихий рассинхрон длительностей, а на длительностях у нас
построена вся история диалога.
...
`reason`: `greeting_cut` (FR-03) · `barge_in` (FR-13) · `closing_cancelled` (FR-26) · `reset`.

`closing_cancelled` — собеседник заговорил во время прощания. Отдельная причина, а не
`barge_in`: там мы уступаем ход по решению LLM, здесь просто отменяем завершение разговора,
и в телеметрии это разные события.
